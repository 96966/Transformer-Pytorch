...
# ==========================================
    ...
    if DEBUG_MODE:
        print("⚠️  进入调试模式：仅使用 10 条数据进行过拟合测试...")
        
        # 1. 创建一个只包含前 10 个索引的 Subset
        # 假设 train_dataset 是你原本定义好的 Dataset 对象
        ...
        # 3. 强制减少 Epoch 数 (因为数据少，需要多跑几轮才能背下来)
        NUM_EPOCHS = 500  # 原来是 10？改成 200！
        
        # 打印一下这 10 条数据是什么，方便后面核对
        print("\n--- 待背诵的 10 条黄金数据 ---")
        for i in range(10):
            s, t = train_dataset[i]
            print(f"{i}: {s[:30]}... -> {t[:30]}...")
        ...
        if DEBUG_MODE and (epoch + 1) % 20 == 0: # 每 20 个 epoch 打印一次，避免刷屏
            ...
            with torch.no_grad():
                ...
                for i in range(10):
                    ...
                    # 2. 运行模型推理 (假设你有 translate 或 generate 函数)
                    # 如果没有单独的推理函数，这里需要手动写一个简单的 greedy search
                    # 下面是一个简化的 greedy decode 示例：
                    ...
                    for _ in range(max_len):
                        ...
                        # 调用模型 (注意：推理时通常不需要 tgt_mask 的 future masking，或者用全看到的 mask)
                        # 这里假设 model(src, tgt) 返回的是 logits
